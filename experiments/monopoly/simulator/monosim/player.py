...
class Player:
    ...
    def get_state(self):
        ...
        return {
            ...
            "cash": self._cash,
            "mortgageable_amount": self._properties_total_mortgageable_amount,
            "jail_count": self._jail_count,
            "exit_jail": self._exit_jail,
            "free_visit": self._free_visit,
            "owned_roads": self._list_owned_roads,
            "owned_stations": self._list_owned_stations,
            "owned_utilities": self._list_owned_utilities,
            "mortgaged_roads": self._list_mortgaged_roads,
            "mortgaged_stations": self._list_mortgaged_stations,
            "mortgaged_utilities": self._list_mortgaged_utilities,
            "owned_colors": self._dict_owned_colors,
            "owned_houses_hotels": self._dict_owned_houses_hotels,
            "has_lost": self._has_lost,
    ...
    def pay_bank(self, amount):
        ...
        if amount > self._cash:
            raise Exception(
                "Player {} does not have enough money to pay the bank".format(
                    self._name
                )
            )
        # pay bank
    ...
    def buy_property(self, dict_property_info):
        ...
        # mortgage value
        self._properties_total_mortgageable_amount += dict_property_info[
            "mortgage_value"
        ]
    ...
    def mortgage(self, property_name, property_type):
        ...
        if property_type == "station":
            if property_name not in self._list_owned_stations:
                raise Exception(
                    "Station {} not owned by player {}".format(
            ...
            if property_name not in self._list_owned_utilities:
                raise Exception(
                    "Utility {} not owned by player {}".format(
                        property_name, self._name
                    )
                )
            self._dict_properties[property_name]["is_mortgaged"] = True
            ...
            if property_name not in self._list_owned_roads:
                raise Exception(
                    "Road {} not owned by player {}".format(property_name, self._name)
                )
    ...
    def unmortgage(self, property_name, property_type):
        ...
        if property_type == "road":
            if property_name not in self._list_mortgaged_roads:
                raise Exception(
                    "Road {} not mortgaged by player {}".format(
                        property_name, self._name
                    )
                )
            ...
            if property_name not in self._list_mortgaged_utilities:
                raise Exception(
                    "Utility {} not mortgaged by player {}".format(
                        property_name, self._name
                    )
                )
            ...
            if property_name not in self._list_mortgaged_stations:
                raise Exception(
                    "Station {} not mortgaged by player {}".format(
                        property_name, self._name
                    )
                )
            self._dict_properties[property_name]["is_mortgaged"] = False
    ...
    def choose_mortgage_properties(self, amount):
        ...
        list_mortgageable_roads = [
            ("road", road_name, self._dict_roads[road_name]["mortgage_value"])
            for road_name in self._list_owned_roads
            if self._dict_roads[road_name]["is_mortgaged"] is False
        ]
        list_mortgageable_stations = [
            (
                "station",
                station_name,
                self._dict_properties[station_name]["mortgage_value"],
            )
            for station_name in self._list_owned_stations
            if self._dict_properties[station_name]["is_mortgaged"] is False
        ]
        list_mortgageable_utilities = [
            (
                "utility",
                utility_name,
                self._dict_properties[utility_name]["mortgage_value"],
            )
        ...
        if len(list_mortgageable_properties) == 0:
            raise Exception(
                "player {} has no properties to mortgage".format(self._name)
            )

        idx_property = 1
        mortgage_value = 0
        list_properties = []
        while (mortgage_value < amount) and (
            idx_property <= len(list_mortgageable_properties)
        ):
            tuple_tmp = (
                list_mortgageable_properties[idx_property - 1][0],
                list_mortgageable_properties[idx_property - 1][1],
            )
        ...
        if mortgage_value < amount:
            raise Exception(
                "player {} total mortgageable amount is insufficient".format(self._name)
            )
    ...
    def mortgage_and_pay_rent(self, dict_property_info):
        ...
        if self._cash >= property_rent:
            raise Exception(
                "player {} has money and should not mortgage".format(self._name)
            )
        if self._properties_total_mortgageable_amount + self._cash < property_rent:
            raise Exception(
                "player {} cannot rent (insufficient funds)".format(self._name)
            )
    ...
    def get_owned_colors(self):
        ...
        list_colors = [
            color
            for color in self.color_to_house_mapping.keys()
            if self.has_all_roads_of_color(color)
        ]
        return list_colors
    ...
    def estimate_rent_road(self, dict_property_info):
        ...
        if opponent_name is None:
            raise Exception(
                "{} does not belong to anyone".format(dict_property_info["name"])
            )
        opponent = self._dict_players[opponent_name]
        property_name = dict_property_info["name"]
        ...
        if opponent.has_all_roads_of_color(road_color):
            ...
            if hotel == 0 and houses == 0:
                ...
                rent = dict_property_info[
                    "rent_with_{}_houses_{}_hotels".format(houses, hotel)
                ]
        else:
    ...
    def estimate_rent_station(self, dict_property_info):
        ...
        opponent_name, property_name = (
            dict_property_info["belongs_to"],
            dict_property_info["name"],
        )
        if opponent_name is None:
            raise Exception(
                "{} does not belong to anyone".format(dict_property_info["name"])
    ...
    def estimate_rent_utility(self, dict_property_info):
        ...
        try:
            ...
            print("Player dict: ", self._dict_players)
            raise e
    ...
    def owns_all_roads_of_a_color(self):
        ...
        """
        if (
            self._dict_owned_colors["brown"]
            or self._dict_owned_colors["light_blue"]
            or self._dict_owned_colors["purple"]
            or self._dict_owned_colors["orange"]
            or self._dict_owned_colors["red"]
            or self._dict_owned_colors["yellow"]
            or self._dict_owned_colors["green"]
            or self._dict_owned_colors["blue"]
        ):
            return True
        else:
            return False
    ...
    def buy_house(self, road):
        ...
        if self._dict_owned_houses_hotels[road][0] == 4:
            raise Exception(
                "Player {} already owns 4 houses in the road {}. "
        ...
        if self._bank["houses"] > 0:
            ...
            if self._cash < house_price:
                raise Exception(
                    "Player {} has not enough money to buy a house in road {}".format(
                        self._name, road
                    )
                )
    ...
    def buy_hotel(self, road):
        ...
        if self._dict_owned_houses_hotels[road][1] == 1:
            raise Exception(
                "Player {} already owns 1 hotel in the road {}. "
        ...
        if self._bank["hotels"] > 0:
            ...
            if self._cash < hotel_price:
                raise Exception(
                    "Player {} has not enough money to buy a hotel in road {}".format(
                        self._name, road
                    )
                )
            ...
            if houses_owned != 4:
                raise Exception(
                    "Player {} cannot buy a hotel in road {} if "
    ...
    def pay_jail_or_wait(self):
        ...
        if self._jail_count == 3:
            raise Exception(
                "Player {} has been in jail for three consecutive rounds. Player has to pay and leave the"
                "jail."
            )
    ...
    def play_community_chest(self, board_cell_name):
        ...
        if board_cell_name == "street_repair":
            ...
            board_cell_name == "hospital_fees"
            or board_cell_name == "school_fees"
            or board_cell_name == "doctor_fees"
        ):
    ...
    def play(self):
        ...
        if self._position != 10 or (
            self._position == 10 and self._free_visit
        ...
        if self.owns_all_roads_of_a_color() and self.want_to_buy_house_hotel():
            ...
            if (
                house_or_hotel == "house"
                and self._bank["houses"] > 0
                and self._dict_owned_houses_hotels[road][0] < 4
            ):
                house_price = self._dict_roads[road]["houses_cost"]
                if self.have_enough_money(house_price):
                    ...
                    if self.want_to_mortgage_to_buy_house():
                        if (
                            self._properties_total_mortgageable_amount + self._cash
                            >= house_price
                        ):
                            self.get_money_from_mortgages(house_price)
                            self.buy_house(road)
            elif (
                house_or_hotel == "hotel"
                and self._bank["hotels"] > 0
                and self._dict_owned_houses_hotels[road][1] == 0
            ):
                hotel_price = self._dict_roads[road]["hotels_cost"]
        ...
        if board_cell_type == "jail" and not self._free_visit:
            ...
            board_cell_type == "road"
            or board_cell_type == "station"
            or board_cell_type == "utility"
        ):
            property_name = board_cell["name"]
            dict_property_info = (
                self._dict_roads[property_name]
                if board_cell_type == "road"
                else self._dict_properties[property_name]
            )
            property_owner = dict_property_info["belongs_to"]
            if (
                property_name in self._list_owned_roads
                or property_name in self._list_owned_stations
                or property_name in self._list_owned_utilities
            ):
                pass
                ...
                elif self.have_enough_money(
                    dict_property_info["price"], plus_mortgageable=True
                ):
                    mortgage_bid = self.mortgage_or_bid(dict_property_info)
                    if mortgage_bid == "mortgage":
                        self.mortgage_and_buy(
                            dict_property_info, property_name, board_cell_type
                        )
                    else:
                        self.bid(dict_property_info, "temp")
                else:
                ...
                property_owner is not None
                and dict_property_info["is_mortgaged"] is False
            ):
                # Have enough money to rent?
                rent = self.estimate_rent(dict_property_info)
                if self.have_enough_money(
                    rent
                ):  # TODO should it check money + mortgageable amount?
            ...
            self.community_cards_deck.append(
                self.community_cards_deck.pop(0)
            )  # Put top card at the bottom of the deck
